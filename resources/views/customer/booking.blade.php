            ...
            @if(isset($bookings) && $bookings->isNotEmpty())
                <div class="space-y-4">
                    @foreach($bookings as $booking)
                        <div class="bg-white p-6 rounded-xl shadow-sm hover:shadow-lg transition flex flex-col sm:flex-row sm:justify-between items-start sm:items-center">
                            <div class="flex-1">
                                ...
                                @if($booking->status)
                                    <div class="text-sm mt-1">
                                        Status: 
                                        <span class="font-semibold text-{{ $booking->status == 'confirmed' ? 'green-600' : 'yellow-600' }}">
