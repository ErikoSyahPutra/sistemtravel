<x-customer-layout title="Formulir Booking">
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-8 rounded-xl shadow-lg">
                ...
                <form action="{{ route('customer.booking.store') }}" method="POST" class="space-y-4" id="booking-form">
                    @csrf
                    ...
                    <div>
                        <label for="end_date_display" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Berakhir</label>
                        <input
                            type="text"
                            id="end_date_display"
                            name="end_date_display"
                            value="{{ old('end_date') ? \Carbon\Carbon::parse(old('end_date'))->isoFormat('DD MMM YYYY') : '' }}"
                            readonly
                    ...
                    <div class="bg-gray-50 p-4 rounded-md border border-gray-100">
                        <div class="flex justify-between items-center">
                            ...
                            <div class="text-sm font-semibold text-gray-800">{{ $package->duration }} hari</div>
                        </div>
                        <div class="flex justify-between items-center mt-2">
                            <div class="text-sm text-gray-600">Harga / orang</div>
                            <div class="text-sm font-semibold text-blue-600">Rp<span id="price-per-person">{{ number_format($package->price, 0, ',', '.') }}</span></div>
                        </div>
                        <div class="flex justify-between items-center mt-2">
    ...
    <script>
        (function(){
            const duration = {{ (int) $package->duration }};
            const price = {{ (int) $package->price }};
            const startEl = document.getElementById('start_date');
            const paxEl = document.getElementById('pax');
            const endHiddenEl = document.getElementById('end_date');
            const endDisplayEl = document.getElementById('end_date_display');
